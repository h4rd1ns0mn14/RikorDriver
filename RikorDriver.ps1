    ...
    switch ($Task) {
        "SmartUpdate" {
            ...
            try {
                ...
                $fileId = '14_iaT8zdS800GpL76CSVb5vBQN7whZ8w'
                $initialUrl = "https://docs.google.com/uc?export=download&id=$fileId"

                $response = Invoke-WebRequest -Uri $initialUrl -UseBasicParsing -SessionVariable 'session' -MaximumRedirection 0 -ErrorAction SilentlyContinue

                if ($response.RawContent -match 'name="confirm" value="([^"+]+)"') {
                    $confirmToken = $matches[1]
                    $downloadUrl = "https://docs.google.com/uc?export=download&confirm=$confirmToken&id=$fileId"
                    Write-SilentLog "Confirmation token found. Using download URL: $downloadUrl"
                    Invoke-WebRequest -Uri $downloadUrl -WebSession $session -OutFile $zipPath -UseBasicParsing -ErrorAction Stop
                } else {
                    Invoke-WebRequest -Uri $initialUrl -OutFile $zipPath -UseBasicParsing -ErrorAction Stop
                }
...
function Update-ButtonContainerPadding {
    $totalButtonWidth = 220 + 155 + 120 + 140 + 110 + (4 * 12)
...
function Ensure-PSWindowsUpdateModule {
    ...
    try {
        ...
        $moduleZipUrl = "https://www.powershellgallery.com/api/v2/package/$moduleName"
...
function Start-BackgroundTask {
    ...
    $job = Start-Job -Name $Name -ScriptBlock {
        param($taskName, $logPath, $innerArgs, $jobLanguages, $jobCurrentLanguage, $jobFilterSettings, $scriptDir)
                ...
                "SmartUpdate" {
                    ...
                    try {
                        ...
                        if ($response.RawContent -match 'name="confirm" value="([^"+]+)"') {
                            $confirmToken = $matches[1]
                            $downloadUrl = "https://docs.google.com/uc?export=download&confirm=$confirmToken&id=$fileId"
                            L "Confirmation token found. Using download URL: $downloadUrl"
                            Invoke-WebRequest -Uri $downloadUrl -WebSession $session -OutFile $zipPath -UseBasicParsing -ErrorAction Stop
                        } else {
                            Invoke-WebRequest -Uri $initialUrl -OutFile $zipPath -UseBasicParsing -ErrorAction Stop
                        }
                    ...
                    try {
                        ...
                        if ($null -eq $driverUpdates -or $driverUpdates.Count -eq 0) {
                            ...
                            L (Get-LocalizedString "WU_InitiatingInstall")
                            $driverUpdates | ForEach-Object { L " -> $($_.Title) (Manufacturer: $($_.Manufacturer))" }
    ...
    } -ArgumentList $Name, $log, $TaskArgs, $global:Languages, $global:CurrentLanguage, $global:FilterSettings, $PSScriptRoot
        ...
        if ($global:CurrentTaskLog -and (Test-Path $global:CurrentTaskLog)) {
            ...
            if ($lines) {
                ...
                $form.Invoke([System.Windows.Forms.MethodInvoker]{
                    if($status.Text.Length -ne $text.Length) {
                        $status.Text = $text
...
function Show-FiltersDialog {
    ...
    $txtClass = New-Object Windows.Windows.Forms.TextBox
...
$form.Add_Shown({ Update-ButtonContainerPadding })

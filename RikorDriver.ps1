...
$global:Languages = @{
    "en" = @{
        ...
        PSWU_InstallPrompt = "The Windows Update feature requires the 'PSWindowsUpdate' module. Do you want to install it now? (Requires internet)"
        PSWU_Installing = "Installing PSWindowsUpdate module..."
        PSWU_Installed = "PSWindowsUpdate module installed successfully."
        PSWU_Failed = "Failed to install PSWindowsUpdate module."
    ...
    "ru" = @{
        ...
        PSWU_InstallPrompt = "Для функции обновления Windows требуется модуль 'PSWindowsUpdate'. Установить его сейчас? (Требуется интернет)"
        PSWU_Installing = "Установка модуля PSWindowsUpdate..."
        PSWU_Installed = "Модуль PSWindowsUpdate успешно установлен."
        PSWU_Failed = "Не удалось установить модуль PSWindowsUpdate."
...
function Show-SettingsDialog {
    ...
    $btnApply.Cursor = [System.Windows.Forms.Cursors]::Hand
    $btnApply.Add_Click({
        Set-ProxySettings -ProxyAddr $txtProxy.Text -Enable ($txtProxy.Text -ne "")
        if ($txtProxy.Text) { Add-StatusUI $form $status "Proxy set: $($txtProxy.Text)" }
        else { Add-StatusUI $form $status "Proxy cleared." }
        $settingsForm.Close()
    })
    $btnPanel.Controls.Add($btnApply)
    $btnClear = New-Object Windows.Forms.Button
    ...
    $btnClear.Add_Click({
        $txtProxy.Text = ""
        Set-ProxySettings -ProxyAddr "" -Enable $false
        Add-StatusUI $form $status "Proxy cleared."
    })
    $btnPanel.Controls.Add($btnClear)
    $btnClose = New-Object Windows.Forms.Button
    $btnClose.Text = Get-LocalizedString "Close"
